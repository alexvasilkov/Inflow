...
class OpRunWithStateTest : BaseTest() {
    ...
    @Suppress("BlockingMethodInNonBlockingContext") // Simulating real blocking calls
    @Test
    fun `Loader scope is cancelled if blocking`(): Unit = runBlocking(Dispatchers.IO) {
        val error = MutableStateFlow<Throwable?>(null)
        val action = suspend {
            Thread.sleep(50L)
            throw RuntimeException()
        ...
        val job = launch(Dispatchers.IO) { runWithStateDefaults(error = error, action = action) }
