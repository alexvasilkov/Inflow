...
class OpScheduleUpdatesTest : BaseTest() {
    ...
    fun `Activation flow controls the loading`() = runBlockingTestWithJob { job ->
        val activation = MutableStateFlow(false)
        ...
        assertEquals(expected = 0, actual = counter, "Not loading if not activated")

        delay(200L)
        assertEquals(expected = 0, actual = counter, "Still no loading")

        activation.emit(true)
        assertEquals(expected = 1, actual = counter, "Loading triggered")

        activation.emit(false)
        delay(400L)
        assertEquals(expected = 1, actual = counter, "Not loading while inactive")
    }
