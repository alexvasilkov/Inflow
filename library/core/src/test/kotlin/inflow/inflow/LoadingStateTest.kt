...
class LoadingStateTest : BaseTest() {
    ...
    @Test
    fun `IF data is subscribed THEN loading is triggered`() = runTest { job ->
        val tracker = TestTracker()
        val inflow = testInflow {}
    ...
    @Test
    fun `IF data is subscribed with CacheOnly THEN loading is not triggered`() = runTest { job ->
        val tracker = TestTracker()
        val inflow = testInflow {}

        launch(job) { inflow.loading().track(tracker) }
